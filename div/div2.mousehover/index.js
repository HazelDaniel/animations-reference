...
document.addEventListener("DOMContentLoaded", ()=>{
	let calculateAngle = function (e, item, parent) {
		...
		if (parent.getAttribute("data-filter-color")) {
		...
		let calcAngleX = (x - halfWidth) / 20;
		let calcAngleY = (y - halfHeight) / 25;

		let gX = (1 - x / (halfWidth * 2)) * 100;
		let gY = (1 - y / (halfHeight * 2)) * 100;

		item.querySelector(".glare")
		.style.background = `radial-gradient(circle at ${gX}% ${gY}%, rgb(199 198 243), transparent)`;

		// And set its container's perspective.
		parent.style.perspective = `${halfWidth * 20}px`;
		item.style.perspective = `${halfWidth * 20}px`;
		...
		let calcShadowX = (x - halfWidth) / 6;
		let calcShadowY = (y - halfHeight) / 12;
	...
	card.addEventListener("mouseleave", (e) => {
		...
		card.querySelector( ".inner-card-backface").style.transform =
